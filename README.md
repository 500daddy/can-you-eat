...
部署 `foodApi` 和 `mockRecognize` 后，页面会优先调用云函数；如果云函数失败，会自动回退本地数据，方便调试不中断。拍照识别会优先上传到云存储并调用 `mockRecognize`，云函数配置 `DASHSCOPE_API_KEY` 或 `QWEN_API_KEY` 后会把图片交给 Qwen 视觉模型识别，未配置或调用失败时回退本地模拟识别。
